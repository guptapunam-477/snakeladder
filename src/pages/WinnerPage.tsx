...
export default function WinnerPage({ room, myId, isHost, dispatch, onLeave }: Props) {
    ...
    <div className="relative mx-auto flex min-h-[100dvh] max-w-md flex-col gap-5 px-5 py-10">
      ...
      <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
        ...
        <ul className="flex flex-col gap-1.5">
          {ranked.map((p, i) => (
            <li key={p.id} className="flex items-center gap-2 rounded-lg bg-white/5 px-3 py-2">
              ...
              <span className="flex-1 truncate font-semibold text-white">
                {p.name}
                {p.id === myId && <span className="ml-1 text-[10px] text-emerald-300">(you)</span>}
              </span>
